    ...
    <div class="page-header">
        ...
        <div class="ml-auto">
            <div class="input-group">
                <label for="route_id" class="col-form-label mr-2">Filter by Route *:</label>
                <select class="form-control mr-2" aria-label="Default select example" name="route_id" id="route">
                    ...
                    @endphp
                        ...
                        @php
                        $blank_point .=$route->pickup_point_id.',';
                        @endphp
        ...
        <div class="col-md-12 col-lg-12">
            <div class="card">
                <div class="card-body pt-4">
                    ...
                    <form name="ajax_form" method="post" action="{{ route('assign-bus.store') }}"
                        enctype="multipart/form-data" novalidate>
                        @csrf
                        @method('post')
                        <table class="table table-bordered table-striped table-highlight text-center" id="get_routes">
                            <thead>
                                <tr>
                                    <th>
                            ...
                            <tbody>
                                @php
                                $newarraynama =explode(',', rtrim($blank_point, ", "));
                                $b = array_map('trim', $newarraynama);
                                    $i = 1;
                                    $num = 0;
                                @endphp
                                @foreach ($points as $item)
                                @if (in_array($item->id,$b))
                                    
                                    <tr>  
                                        <td>
                                          <input type="checkbox" class="checkhour" value="{{$item->id}}" name="route_checkbox[]">
                                        ...
                                        <td>
                                            {{ $item->name }}
                                            <input type="hidden" name="pickup_point_id[]" class="pickup_point_id"
                                                value="{{ $item->id }}" id="pickup_point_id">
                                        </td>
                                        <td>
                                            {{ $item->seat_count }}
                                        ...
                                        <td>
                                            <select name="date_concert[]" class="form-control date_concert"
                                                ...
                                                @foreach ($item->date_concert as $date)
                                                    @if (isset($item->bus_assign[0]->schedule_date) ? $item->bus_assign[0]->schedule_date : 0 == trim($date))
                                                        <option value="{{ $date }}" selected>
                                                            {{ date('d/m/Y', strtotime($date)) }}</option>
                                                    @else
                                                        <option value="{{ $date }}">
                                                            {{ date('d/m/Y', strtotime($date)) }}</option>
                                                    @endif
                                        ...
                                        <td>
                                            <select name="buses[]" class="form-control buses"
                                                ...
                                                @foreach ($buses as $bus)
                                                    ...
                                                    @else
                                                        <option value="{{ $bus->id }}">{{ $bus->bus_number }}
                                                        </option>
                                                    @endif
                                                @endforeach
                                            </select>
